"""
DeepSeek-V4 对话编码（chat templating）。

直接整合用户提供的 encoding 模块，只做轻微调整：
- 从 special_tokens.py 引用常量，避免重复定义
- 暴露统一的 encode/decode/parse 接口
- 支持与 DeepseekV4Tokenizer 集成
"""
...
from deepseek_v4.tokenizer.special_tokens import (
    ASSISTANT_TOKEN as ASSISTANT_SP_TOKEN,
    BOS_TOKEN as bos_token,
    DSML_TOKEN as dsml_token,
    EOS_TOKEN as eos_token,
    LATEST_REMINDER_TOKEN as LATEST_REMINDER_SP_TOKEN,
    TASK_TOKENS as DS_TASK_SP_TOKENS,
    THINK_END as thinking_end_token,
    THINK_START as thinking_start_token,
    USER_TOKEN as USER_SP_TOKEN,
    VALID_TASKS,
)
...
def encode_arguments_to_dsml(tool_call: Dict[str, str]) -> str:
    ...
    for k, v in arguments.items():
        parts.append(p_dsml_template.format(
...
def render_message(
    ...
    assert reasoning_effort in ["max", None, "high"], f"Invalid reasoning_effort: {reasoning_effort}"
    ...
    if role == "system":
        ...
        assert content, f"Invalid developer message: {msg}"
        c = USER_SP_TOKEN + content
        if tools:
            c += "\n\n" + render_tools(tools)
        if response_format:
            c += "\n\n" + response_format_template.format(schema=to_json(response_format))
        prompt += user_msg_template.format(content=c)
            ...
            for block in content_blocks:
                ...
                if bt == "text":
                    ...
                    if isinstance(tc, list):
                        ...
                        tc = "\n\n".join(text_parts)
                    parts.append(tool_output_template.format(content=tc))
                else:
                    parts.append(f"[Unsupported {bt}]")
            prompt += "\n\n".join(parts)
        else:
            prompt += content or ""
    ...
    for msg in messages:
        ...
        if role == "tool":
            ...
            if (merged and merged[-1].get("role") == "user"
                ...
                new_msg = {"role": "user", "content": msg.get("content", ""), "content_blocks": [text_block]}
...
def parse_tool_calls(index: int, text: str) -> Tuple[int, Optional[str], List[Dict[str, str]]]:
    ...
    while index < len(text):
        index, content, stop_token = _read_until_stop(index, text, [f"<{dsml_token}invoke", tool_calls_end_token])
        if content != ">\n":
            raise ValueError(f"Tool call format error: '{content}'")
...
def parse_message_from_completion_text(text: str, thinking_mode: str) -> Dict[str, Any]:
    ...
    for sp in [bos_token, eos_token, thinking_start_token, thinking_end_token, dsml_token]:
        assert sp not in summary_content and sp not in reasoning_content, f"Unexpected '{sp}' in content"
    return {
        "role": "assistant",
