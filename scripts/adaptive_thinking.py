...
用法：
    python scripts/adaptive_thinking.py --config configs/training/adaptive_thinking.yaml
"""
...
from deepseek_v4.training.adaptive_thinking import (
...
logger = get_logger("adaptive_thinking")
...
def main():
    parser = argparse.ArgumentParser("DeepSeek-V4 Mini Adaptive Thinking")
    parser.add_argument("--config", required=True)
    parser.add_argument("overrides", nargs="*")
    args = parser.parse_args()

    cfg = load_config_with_overrides(AdaptiveThinkingConfig, args.config, args.overrides)
    ...
    trainer = AdaptiveThinkingTrainer(config=cfg, model=model, tokenizer=tokenizer)
    trainer.train()
