...
from deepseek_v4.training.reward_model import RewardConfig, RewardTrainer
...
logger = get_logger("reward_model")
...
def main():
    parser = argparse.ArgumentParser("DeepSeek-V4 Mini Reward Model Training")
    parser.add_argument("--config", required=True)
    parser.add_argument("overrides", nargs="*")
    args = parser.parse_args()

    cfg = load_config_with_overrides(RewardConfig, args.config, args.overrides)
    ...
    trainer = RewardTrainer(config=cfg, model=model, tokenizer=tokenizer)
    trainer.train()
