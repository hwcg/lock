...
用法：
    python scripts/agentic_rl.py --config configs/training/agentic_rl.yaml
"""
...
from deepseek_v4.training.agentic_rl import AgenticRLConfig, AgenticRLTrainer
...
def main():
    parser = argparse.ArgumentParser("DeepSeek-V4 Mini Agentic RL")
    parser.add_argument("--config", required=True)
    parser.add_argument("overrides", nargs="*")
    args = parser.parse_args()

    cfg = load_config_with_overrides(AgenticRLConfig, args.config, args.overrides)
    ...
    trainer = AgenticRLTrainer(config=cfg, policy=policy, tokenizer=tokenizer)
    trainer.train()
