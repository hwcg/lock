#!/usr/bin/env python
"""
评测入口

用法：
    python scripts/evaluate.py --config configs/eval_config.yaml --task ceval
    python scripts/evaluate.py --task gsm8k --backend local --model_path checkpoints/sft/checkpoint-final
"""
from __future__ import annotations

import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(ROOT / "src"))

import argparse

from deepseek_v4.evaluation.engine import LocalEngine, OpenAIEngine, VLLMEngine
from deepseek_v4.evaluation.base import Evaluator, EvalConfig
from deepseek_v4.tokenizer.tokenizer import DeepseekV4Tokenizer
from deepseek_v4.modeling.model import DeepseekV4Config, DeepseekV4ForCausalLM
from deepseek_v4.utils.logger import setup_logging


# Built-in evaluation tasks
from deepseek_v4.evaluation.tasks.base_task import CEvalTask, CMMLUTask, GSM8KTask, HumanEvalTask, OpenBookQATask

TASK_MAP = {
    "ceval": CEvalTask,
    "cmmlu": CMMLUTask,
    "gsm8k": GSM8KTask,
    "humaneval": HumanEvalTask,
    "openbookqa": OpenBookQATask,
}


def main():
    parser = argparse.ArgumentParser("DeepSeek-V4 Mini Evaluation")
    parser.add_argument("--task", required=True, choices=list(TASK_MAP.keys()))
    parser.add_argument("--backend", default="local", choices=["local", "openai", "vllm"])
    parser.add_argument("--model_path", default=None, help="Path to model checkpoint")
    parser.add_argument("--tokenizer_path", default=None, help="Path to tokenizer")
    parser.add_argument("--config", default=None, help="Optional YAML eval config")
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--dtype", default="bfloat16")
    parser.add_argument("--max_seq_len", type=int, default=4096)
    parser.add_argument("--openai_base_url", default="http://localhost:8000/v1")
    parser.add_argument("--openai_api_key", default="EMPTY")
    parser.add_argument("--openai_model", default="deepseek-v4-mini")
    parser.add_argument("--output_dir", default="eval_results")
    args = parser.parse_args()

    setup_logging(level="INFO")

    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)

    # Build engine
    if args.backend == "local":
        if not args.model_path or not args.tokenizer_path:
            parser.error("--model_path and --tokenizer_path required for local backend")
        import torch
        import json
        tokenizer = DeepseekV4Tokenizer.from_pretrained(args.tokenizer_path)
        ckpt_dir = Path(args.model_path)
        config_file = ckpt_dir.parent / "config.json" if ckpt_dir.is_dir() else ckpt_dir.parent.parent / "config.json"
        if config_file.exists():
            with open(config_file, "r") as f:
                model_cfg = DeepseekV4Config.from_dict(json.load(f))
        else:
            model_cfg = DeepseekV4Config()
        model = DeepseekV4ForCausalLM(model_cfg)
        from safetensors.torch import load_file
        sd = load_file(str(ckpt_dir / "model.safetensors")) if ckpt_dir.is_dir() else torch.load(str(ckpt_dir), map_location="cpu") if args.model_path.endswith(".bin") else load_file(str(args.model_path))
        model.load_state_dict(sd, strict=False)
        engine = LocalEngine(model=model, tokenizer=tokenizer, device=args.device, dtype=args.dtype)
    elif args.backend == "openai":
        engine = OpenAIEngine(base_url=args.openai_base_url, api_key=args.openai_api_key, model=args.openai_model)
    elif args.backend == "vllm":
        engine = VLLMEngine(model_path=args.model_path, max_seq_len=args.max_seq_len)
    else:
        raise ValueError(f"Unknown backend: {args.backend}")

    # Run
    task_cls = TASK_MAP[args.task]
    task = task_cls()
    config = EvalConfig()
    evaluator = Evaluator(engine=engine, tasks=[task], config=config)
    results = evaluator.evaluate()
    evaluator.save_results(results, str(out))
    evaluator.print_summary(results)


if __name__ == "__main__":
    main()
