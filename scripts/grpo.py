...
用法：
    python scripts/grpo.py --config configs/training/grpo.yaml
"""
...
from deepseek_v4.training.grpo import GRPOConfig, GRPOTrainer
...
def main():
    parser = argparse.ArgumentParser("DeepSeek-V4 Mini GRPO")
    parser.add_argument("--config", required=True)
    parser.add_argument("overrides", nargs="*")
    args = parser.parse_args()

    cfg = load_config_with_overrides(GRPOConfig, args.config, args.overrides)
    ...
    trainer = GRPOTrainer(config=cfg, policy=policy, tokenizer=tokenizer, reward_fns=rewards)
    trainer.train()
